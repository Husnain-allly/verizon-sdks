# verizon
#
# This file was automatically generated by APIMATIC v2.0
# ( https://apimatic.io ).

module Verizon
  # DataTriggerRequest Model.
  class DataTriggerRequest < BaseModel
    SKIP = Object.new
    private_constant :SKIP

    # TODO: Write general description for this method
    # @return [String]
    attr_accessor :comparator

    # TODO: Write general description for this method
    # @return [Integer]
    attr_accessor :threshold

    # TODO: Write general description for this method
    # @return [String]
    attr_accessor :threshold_unit

    # A mapping from model property names to API property names.
    def self.names
      @_hash = {} if @_hash.nil?
      @_hash['comparator'] = 'comparator'
      @_hash['threshold'] = 'threshold'
      @_hash['threshold_unit'] = 'thresholdUnit'
      @_hash
    end

    # An array for optional fields
    def self.optionals
      %w[
        comparator
        threshold
        threshold_unit
      ]
    end

    # An array for nullable fields
    def self.nullables
      []
    end

    def initialize(comparator: SKIP, threshold: SKIP, threshold_unit: SKIP)
      @comparator = comparator unless comparator == SKIP
      @threshold = threshold unless threshold == SKIP
      @threshold_unit = threshold_unit unless threshold_unit == SKIP
    end

    # Creates an instance of the object from a hash.
    def self.from_hash(hash)
      return nil unless hash

      # Extract variables from the hash.
      comparator = hash.key?('comparator') ? hash['comparator'] : SKIP
      threshold = hash.key?('threshold') ? hash['threshold'] : SKIP
      threshold_unit = hash.key?('thresholdUnit') ? hash['thresholdUnit'] : SKIP

      # Create object from extracted values.
      DataTriggerRequest.new(comparator: comparator,
                             threshold: threshold,
                             threshold_unit: threshold_unit)
    end

    # Provides a human-readable string representation of the object.
    def to_s
      class_name = self.class.name.split('::').last
      "<#{class_name} comparator: #{@comparator}, threshold: #{@threshold}, threshold_unit:"\
      " #{@threshold_unit}>"
    end

    # Provides a debugging-friendly string with detailed object information.
    def inspect
      class_name = self.class.name.split('::').last
      "<#{class_name} comparator: #{@comparator.inspect}, threshold: #{@threshold.inspect},"\
      " threshold_unit: #{@threshold_unit.inspect}>"
    end
  end
end
