...
```ts
const softwareManagementLicensesV3Api = new SoftwareManagementLicensesV3Api(client);
```
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV3Api.getAccountLicensesStatus(
  acc,
  lastSeenDeviceId
);
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV3Api.assignLicensesToDevices(
  acc,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV3Api.removeLicensesFromDevices(
  acc,
  body
);
  // Get more response info...
