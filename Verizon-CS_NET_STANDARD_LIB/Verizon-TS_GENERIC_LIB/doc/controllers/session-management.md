...
```ts
const sessionManagementApi = new SessionManagementApi(client);
```
...
try {
  const { result, ...httpResponse } = await sessionManagementApi.startConnectivityManagementSession(body);
...
```ts
try {
  const { result, ...httpResponse } = await sessionManagementApi.endConnectivityManagementSession();
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await sessionManagementApi.resetConnectivityManagementPassword(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
