...
```ts
const softwareManagementLicensesV1Api = new SoftwareManagementLicensesV1Api(client);
```
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV1Api.assignLicensesToDevices(
  account,
  body
);
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV1Api.removeLicensesFromDevices(
  account,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV1Api.createListOfLicensesToRemove(
  account,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV1Api.deleteListOfLicensesToRemove(account);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await softwareManagementLicensesV1Api.listLicensesToRemove(
  account,
  startIndex
);
  // Get more response info...
