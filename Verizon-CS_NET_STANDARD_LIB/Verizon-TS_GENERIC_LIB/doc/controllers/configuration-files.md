...
```ts
const configurationFilesApi = new ConfigurationFilesApi(client);
```
...
try {
  const { result, ...httpResponse } = await configurationFilesApi.getListOfFiles(
  acc,
  distributionType
);
...
try {
  const { result, ...httpResponse } = await configurationFilesApi.uploadConfigFile(
  acc,
  undefined,
  fileVersion,
  make,
