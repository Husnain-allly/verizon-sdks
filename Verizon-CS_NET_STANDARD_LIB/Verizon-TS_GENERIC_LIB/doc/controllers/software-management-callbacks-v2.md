...
```ts
const softwareManagementCallbacksV2Api = new SoftwareManagementCallbacksV2Api(client);
```
...
try {
  const { result, ...httpResponse } = await softwareManagementCallbacksV2Api.listRegisteredCallbacks(account);
...
try {
  const { result, ...httpResponse } = await softwareManagementCallbacksV2Api.updateCallback(
  account,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await softwareManagementCallbacksV2Api.registerCallback(
  account,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await softwareManagementCallbacksV2Api.deregisterCallback(account);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
