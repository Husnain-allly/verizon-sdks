...
import { UpgradeStatus, upgradeStatusSchema } from '../models/upgradeStatus';
import { array, string } from '../schema';
import { BaseApi } from './baseApi';
import { FotaV1ResultError } from '../errors/fotaV1ResultError';

export class SoftwareManagementReportsV1Api extends BaseApi {
  /**
   ...
   * @param startIndex    The zero-based number of the first record to return. Set startIndex=0 for
   *                                       the first request. If `hasMoreFlag`=true in the response, use the
   *                                       `lastSeenUpgradeId` value from the response as the startIndex in the next
   *                                       request.
   * @return Response from the API call
   */
  async listUpgradesForSpecifiedStatus(
    ...
    const mapped = req.prepareArgs({
      account: [account, string()],
      upgradeStatus: [upgradeStatus, upgradeStatusSchema],
      startIndex: [startIndex, string()],
    });
