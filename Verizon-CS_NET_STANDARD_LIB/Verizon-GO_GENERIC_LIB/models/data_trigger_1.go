...
// DataTrigger1 represents a DataTrigger1 struct.
type DataTrigger1 struct {
    FilterCriteria       *AccountLevelFilter    `json:"filterCriteria,omitempty"`
    Condition            *DataTrigger1Condition `json:"condition,omitempty"`
...
// String implements the fmt.Stringer interface for DataTrigger1,
// providing a human-readable string representation useful for logging, debugging or displaying information.
func (d DataTrigger1) String() string {
    return fmt.Sprintf(
    	"DataTrigger1[FilterCriteria=%v, Condition=%v, Action=%v, ConditionType=%v, Comparitor=%v, Threshold=%v, ThresholdUnit=%v, CycleType=%v, AllowanceThreshold=%v, AdditionalProperties=%v]",
    	d.FilterCriteria, d.Condition, d.Action, d.ConditionType, d.Comparitor, d.Threshold, d.ThresholdUnit, d.CycleType, d.AllowanceThreshold, d.AdditionalProperties)
}

// MarshalJSON implements the json.Marshaler interface for DataTrigger1.
// It customizes the JSON marshaling process for DataTrigger1 objects.
func (d DataTrigger1) MarshalJSON() (
    []byte,
    error) {
    if err := DetectConflictingProperties(d.AdditionalProperties,
        "filterCriteria", "condition", "action", "conditionType", "comparitor", "threshold", "thresholdUnit", "cycleType", "allowanceThreshold"); err != nil {
        return []byte{}, err
    }
    return json.Marshal(d.toMap())
}

// toMap converts the DataTrigger1 object to a map representation for JSON marshaling.
func (d DataTrigger1) toMap() map[string]any {
...
// UnmarshalJSON implements the json.Unmarshaler interface for DataTrigger1.
// It customizes the JSON unmarshaling process for DataTrigger1 objects.
func (d *DataTrigger1) UnmarshalJSON(input []byte) error {
    var temp tempDataTrigger1
    err := json.Unmarshal(input, &temp)
    if err != nil {
    	return err
    }
    additionalProperties, err := ExtractAdditionalProperties[interface{}](input, "filterCriteria", "condition", "action", "conditionType", "comparitor", "threshold", "thresholdUnit", "cycleType", "allowanceThreshold")
    if err != nil {
    	return err
    }
...
// tempDataTrigger1 is a temporary struct used for validating the fields of DataTrigger1.
type tempDataTrigger1  struct {
    FilterCriteria     *AccountLevelFilter    `json:"filterCriteria,omitempty"`
    Condition          *DataTrigger1Condition `json:"condition,omitempty"`
    Action             *AccountLevelAction    `json:"action,omitempty"`
    ConditionType      *ConditionType         `json:"conditionType,omitempty"`
