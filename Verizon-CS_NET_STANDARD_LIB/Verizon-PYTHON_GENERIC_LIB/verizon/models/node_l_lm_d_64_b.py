...
class NodeLLmD64B(object):
    ...
    def validate(cls, dictionary):
        ...
        if isinstance(dictionary, cls):
            return APIHelper.is_valid_type(value=dictionary.lon,
                                           type_callable=lambda value: isinstance(value, int)) \
                and APIHelper.is_valid_type(value=dictionary.lat,
                                            type_callable=lambda value: isinstance(value, int))

        if not isinstance(dictionary, dict):
            return False

        return APIHelper.is_valid_type(value=dictionary.get('lon'),
                                       type_callable=lambda value: isinstance(value, int)) \
            and APIHelper.is_valid_type(value=dictionary.get('lat'),
                                        type_callable=lambda value: isinstance(value, int))
