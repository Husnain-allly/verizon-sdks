...
module Verizon
  ...
  class OffsetSystem < BaseModel
    ...
    # @param [OffsetSystem | Hash] The value against the validation is performed.
    def self.validate(value)
      if value.instance_of? self
        return APIHelper.valid_type?(value.offset,
                                     ->(val) { Offset.validate(val) },
                                     is_model_hash: true)
      end

      return false unless value.instance_of? Hash

      APIHelper.valid_type?(value['offset'],
                            ->(val) { Offset.validate(val) },
                            is_model_hash: true)
    end
