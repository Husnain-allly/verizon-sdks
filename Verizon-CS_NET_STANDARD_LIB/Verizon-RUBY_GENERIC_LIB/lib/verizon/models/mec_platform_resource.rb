...
module Verizon
  ...
  class MecPlatformResource < BaseModel
    ...
    def initialize(ern: SKIP, zone: SKIP, region: SKIP,
                   status: MecPlatformStatus::UNKNOWN, properties: SKIP)
    ...
    def self.from_hash(hash)
      ...
      status = hash['status'] ||= MecPlatformStatus::UNKNOWN
      properties = hash.key?('properties') ? APIHelper.deserialize_union_type(
        UnionTypeLookUp.get(:MecPlatformResourceProperties), hash['properties']
      ) : SKIP

      # Create object from extracted values.
      MecPlatformResource.new(ern: ern,
                              zone: zone,
                              region: region,
                              status: status,
    ...
    # @param [MecPlatformResource | Hash] The value against the validation is performed.
    ...
    def to_s
      ...
      "<#{class_name} ern: #{@ern}, zone: #{@zone}, region: #{@region}, status: #{@status},"\
      " properties: #{@properties}>"
    end
    ...
    def inspect
      class_name = self.class.name.split('::').last
      "<#{class_name} ern: #{@ern.inspect}, zone: #{@zone.inspect}, region: #{@region.inspect},"\
      " status: #{@status.inspect}, properties: #{@properties.inspect}>"
    end
  end
end
