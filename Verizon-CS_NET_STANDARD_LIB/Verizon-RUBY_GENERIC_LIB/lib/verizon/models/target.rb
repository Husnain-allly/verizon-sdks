# verizon
#
# This file was automatically generated by APIMATIC v2.0
# ( https://apimatic.io ).

module Verizon
  # Target resource definition.
  class Target < BaseModel
    SKIP = Object.new
    private_constant :SKIP

    # The endpoint for data streams.
    # @return [String]
    attr_accessor :address

    # The transport format.
    # @return [String]
    attr_accessor :addressscheme

    # The billing account ID.
    # @return [String]
    attr_accessor :billingaccountid

    # The date the resource was created.
    # @return [String]
    attr_accessor :createdon

    # Security identification string.
    # @return [String]
    attr_accessor :externalid

    # ThingSpace unique ID for the target that was created.
    # @return [String]
    attr_accessor :id

    # Identifies the resource kind. Targets are ts.target.
    # @return [String]
    attr_accessor :kind

    # The date the resource was last updated.
    # @return [String]
    attr_accessor :lastupdated

    # Name of the target.
    # @return [String]
    attr_accessor :name

    # AWS region value.
    # @return [String]
    attr_accessor :region

    # Version of the underlying schema resource.
    # @return [String]
    attr_accessor :version

    # The version of the resource.
    # @return [String]
    attr_accessor :versionid

    # Description of the target.
    # @return [String]
    attr_accessor :description

    # A mapping from model property names to API property names.
    def self.names
      @_hash = {} if @_hash.nil?
      @_hash['address'] = 'address'
      @_hash['addressscheme'] = 'addressscheme'
      @_hash['billingaccountid'] = 'billingaccountid'
      @_hash['createdon'] = 'createdon'
      @_hash['externalid'] = 'externalid'
      @_hash['id'] = 'id'
      @_hash['kind'] = 'kind'
      @_hash['lastupdated'] = 'lastupdated'
      @_hash['name'] = 'name'
      @_hash['region'] = 'region'
      @_hash['version'] = 'version'
      @_hash['versionid'] = 'versionid'
      @_hash['description'] = 'description'
      @_hash
    end

    # An array for optional fields
    def self.optionals
      %w[
        address
        addressscheme
        billingaccountid
        createdon
        externalid
        id
        kind
        lastupdated
        name
        region
        version
        versionid
        description
      ]
    end

    # An array for nullable fields
    def self.nullables
      []
    end

    def initialize(address: SKIP, addressscheme: SKIP, billingaccountid: SKIP,
                   createdon: SKIP, externalid: SKIP, id: SKIP, kind: SKIP,
                   lastupdated: SKIP, name: SKIP, region: SKIP, version: SKIP,
                   versionid: SKIP, description: SKIP,
                   additional_properties: nil)
      # Add additional model properties to the instance
      additional_properties = {} if additional_properties.nil?

      @address = address unless address == SKIP
      @addressscheme = addressscheme unless addressscheme == SKIP
      @billingaccountid = billingaccountid unless billingaccountid == SKIP
      @createdon = createdon unless createdon == SKIP
      @externalid = externalid unless externalid == SKIP
      @id = id unless id == SKIP
      @kind = kind unless kind == SKIP
      @lastupdated = lastupdated unless lastupdated == SKIP
      @name = name unless name == SKIP
      @region = region unless region == SKIP
      @version = version unless version == SKIP
      @versionid = versionid unless versionid == SKIP
      @description = description unless description == SKIP
      @additional_properties = additional_properties
    end

    # Creates an instance of the object from a hash.
    def self.from_hash(hash)
      return nil unless hash

      # Extract variables from the hash.
      address = hash.key?('address') ? hash['address'] : SKIP
      addressscheme = hash.key?('addressscheme') ? hash['addressscheme'] : SKIP
      billingaccountid =
        hash.key?('billingaccountid') ? hash['billingaccountid'] : SKIP
      createdon = hash.key?('createdon') ? hash['createdon'] : SKIP
      externalid = hash.key?('externalid') ? hash['externalid'] : SKIP
      id = hash.key?('id') ? hash['id'] : SKIP
      kind = hash.key?('kind') ? hash['kind'] : SKIP
      lastupdated = hash.key?('lastupdated') ? hash['lastupdated'] : SKIP
      name = hash.key?('name') ? hash['name'] : SKIP
      region = hash.key?('region') ? hash['region'] : SKIP
      version = hash.key?('version') ? hash['version'] : SKIP
      versionid = hash.key?('versionid') ? hash['versionid'] : SKIP
      description = hash.key?('description') ? hash['description'] : SKIP

      # Create a new hash for additional properties, removing known properties.
      new_hash = hash.reject { |k, _| names.value?(k) }

      additional_properties = APIHelper.get_additional_properties(
        new_hash, proc { |value| value }
      )

      # Create object from extracted values.
      Target.new(address: address,
                 addressscheme: addressscheme,
                 billingaccountid: billingaccountid,
                 createdon: createdon,
                 externalid: externalid,
                 id: id,
                 kind: kind,
                 lastupdated: lastupdated,
                 name: name,
                 region: region,
                 version: version,
                 versionid: versionid,
                 description: description,
                 additional_properties: additional_properties)
    end

    # Provides a human-readable string representation of the object.
    def to_s
      class_name = self.class.name.split('::').last
      "<#{class_name} address: #{@address}, addressscheme: #{@addressscheme}, billingaccountid:"\
      " #{@billingaccountid}, createdon: #{@createdon}, externalid: #{@externalid}, id: #{@id},"\
      " kind: #{@kind}, lastupdated: #{@lastupdated}, name: #{@name}, region: #{@region}, version:"\
      " #{@version}, versionid: #{@versionid}, description: #{@description},"\
      " additional_properties: #{@additional_properties}>"
    end

    # Provides a debugging-friendly string with detailed object information.
    def inspect
      class_name = self.class.name.split('::').last
      "<#{class_name} address: #{@address.inspect}, addressscheme: #{@addressscheme.inspect},"\
      " billingaccountid: #{@billingaccountid.inspect}, createdon: #{@createdon.inspect},"\
      " externalid: #{@externalid.inspect}, id: #{@id.inspect}, kind: #{@kind.inspect},"\
      " lastupdated: #{@lastupdated.inspect}, name: #{@name.inspect}, region: #{@region.inspect},"\
      " version: #{@version.inspect}, versionid: #{@versionid.inspect}, description:"\
      " #{@description.inspect}, additional_properties: #{@additional_properties}>"
    end
  end
end
