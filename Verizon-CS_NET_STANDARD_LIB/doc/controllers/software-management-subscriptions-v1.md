...
```csharp
SoftwareManagementSubscriptionsV1Api softwareManagementSubscriptionsV1Api = client.SoftwareManagementSubscriptionsV1Api;
```
...
try
{
    ApiResponse<V1AccountSubscription> result = await softwareManagementSubscriptionsV1Api.GetAccountSubscriptionStatusAsync(account);
}
...
try
{
    ApiResponse<AccountLicenseInfo> result = await softwareManagementSubscriptionsV1Api.GetAccountLicenseStatusAsync(
        account,
        startIndex
    );
}
