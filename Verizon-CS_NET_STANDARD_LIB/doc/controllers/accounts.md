...
try
{
    ApiResponse<Account> result = await accountsApi.GetAccountInformationAsync(aname);
}
...
```csharp
ListAccountStatesAndServicesAsync(
    string aname)
```
...
try
{
    ApiResponse<AccountStatesAndServices> result = await accountsApi.ListAccountStatesAndServicesAsync(aname);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
try
{
    ApiResponse<AccountLeadsResult> result = await accountsApi.ListAccountLeadsAsync(aname);
}
catch (ApiException e)
{
    // TODO: Handle exception here
