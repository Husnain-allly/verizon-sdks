...
class AnomalyTriggersV2Api extends BaseApi
{
    /**
     * Creates the trigger to identify an anomaly.
     *
     * @param array<TriggerType1|ActiveAnomalyIndicator|ActiveTriggerIndicator> $body Request to
    ...
    public function createAnomalyDetectionTriggerV2(array $body): ApiResponse
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::POST, '/m2m/v2/triggers')
            ...
            ->parameters(
                ...
                BodyParam::init($body)
                    ->required()
                    ->strictType('anyOf(TriggerType1,ActiveAnomalyIndicator,ActiveTriggerIndicator)[]')
            );
     ...
     *
     * @param array<TriggerType3|ActiveAnomalyIndicator> $body Request to update existing trigger.
     *
     * @return ApiResponse Response from the API call
     */
    public function updateAnomalyDetectionTriggerV2(array $body): ApiResponse
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::PUT, '/m2m/v2/triggers')
            ...
            ->parameters(
                HeaderParam::init('Content-Type', 'application/json'),
                BodyParam::init($body)->required()->strictType('anyOf(TriggerType3,ActiveAnomalyIndicator)[]')
            );
    ...
    public function listAnomalyDetectionTriggerSettingsV2(string $triggerId): ApiResponse
    {
        $_reqBuilder = $this->requestBuilder(RequestMethod::GET, '/m2m/v2/triggers/{triggerId}')
            ->server(Server::THINGSPACE)
            ->auth(Auth::and('thingspace_oauth', 'VZ-M2M-Token'))
            ->parameters(TemplateParam::init('triggerId', $triggerId)->required());
