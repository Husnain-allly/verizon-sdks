...
```php
$deviceServiceManagementApi = $client->getDeviceServiceManagementApi();
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` method on this instance returns the response data which is of type [`BullseyeServiceResult`](../../doc/models/bullseye-service-result.md).
...
$apiResponse = $deviceServiceManagementApi->getDeviceHyperPreciseStatus(
    $imei,
    $accountNumber
);
```
...
$apiResponse = $deviceServiceManagementApi->updateDeviceHyperPreciseStatus($body);
```
