...
```php
$servicePlansApi = $client->getServicePlansApi();
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `getResult()` method on this instance returns the response data which is of type [`ServicePlan[]`](../../doc/models/service-plan.md).
...
$apiResponse = $servicePlansApi->listAccountServicePlans($aname);
```
