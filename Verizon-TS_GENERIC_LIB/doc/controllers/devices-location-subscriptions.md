...
```ts
const devicesLocationSubscriptionsApi = new DevicesLocationSubscriptionsApi(client);
```
...
try {
  const { result, ...httpResponse } = await devicesLocationSubscriptionsApi.getLocationServiceSubscriptionStatus(accountName);
...
try {
  const { result, ...httpResponse } = await devicesLocationSubscriptionsApi.getLocationServiceUsage(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
