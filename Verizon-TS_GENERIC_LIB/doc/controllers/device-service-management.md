...
```ts
const deviceServiceManagementApi = new DeviceServiceManagementApi(client);
```
...
try {
  const { result, ...httpResponse } = await deviceServiceManagementApi.getDeviceHyperPreciseStatus(
  imei,
  accountNumber
);
...
try {
  const { result, ...httpResponse } = await deviceServiceManagementApi.updateDeviceHyperPreciseStatus(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
