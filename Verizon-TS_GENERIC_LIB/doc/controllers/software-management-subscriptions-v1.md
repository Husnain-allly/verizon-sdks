...
```ts
const softwareManagementSubscriptionsV1Api = new SoftwareManagementSubscriptionsV1Api(client);
```
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `result` property of this instance returns the response data which is of type [V1AccountSubscription](../../doc/models/v1-account-subscription.md).
...
try {
  const { result, ...httpResponse } = await softwareManagementSubscriptionsV1Api.getAccountSubscriptionStatus(account);
...
This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `result` property of this instance returns the response data which is of type [AccountLicenseInfo](../../doc/models/account-license-info.md).
...
try {
  const { result, ...httpResponse } = await softwareManagementSubscriptionsV1Api.getAccountLicenseStatus(
  account,
  startIndex
);
  // Get more response info...
