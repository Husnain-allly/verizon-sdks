...
```ts
const deviceGroupsApi = new DeviceGroupsApi(client);
```
...
try {
  const { result, ...httpResponse } = await deviceGroupsApi.createDeviceGroup(body);
...
try {
  const { result, ...httpResponse } = await deviceGroupsApi.listDeviceGroups(aname);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await deviceGroupsApi.getDeviceGroupInformation(
  aname,
  gname
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await deviceGroupsApi.updateDeviceGroup(
  aname,
  gname,
  body
);
...
try {
  const { result, ...httpResponse } = await deviceGroupsApi.deleteDeviceGroup(
  aname,
  gname
);
  // Get more response info...
